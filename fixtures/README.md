...
Core note: numeric guardrail fixtures (`fixtures/core/numeric/NUM-01*`, `NUM-02*`) are generated deterministically via `scripts/generate_numeric_fixtures.py` using `reference/python/aicp_ref/hashing.py` for hash/chain computation in pass vectors and placeholder hashes for explicit unsafe-integer expected-fail vectors.

Core note: the legacy Part 1 float-fail numeric fixture was retired in M16 Part 2 and replaced by `NUM-01_float_payload_pass.jsonl` + `NUM-02_unsafe_integer_expected_fail.jsonl`.
