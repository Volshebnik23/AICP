...
- **Shipped:** `EXT-IAM-BRIDGE` defines normalized IAM claims snapshot, contract-level mapping policy, and step-up (`acr`/`amr`) evaluation semantics bound to delegated-identity context.
- **Shipped:** deterministic fixtures and executable extension conformance (`IB-IAM-BRIDGE-0.1`) validate issuer allowlist, scope/role/group mapping, binding linkage, step-up checks, and approval-required policy mapping.
- **Next concrete step:** proceed to M27 production attributes (tracing/SLA/metering) while keeping enterprise controls stable.
