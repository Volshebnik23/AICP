...
- **Next concrete step:** keep M27 observability stable alongside shipped M29 enterprise bindings.

### ✅ M29 — Enterprise domain bindings (OpenAPI/OData/OPA/ABAC)
- **Shipped:** `EXT-ENTERPRISE-BINDINGS` RFC + strict schema + deterministic fixtures + executable extension conformance (`EB-ENTERPRISE-BINDINGS-0.1`) are in-repo and wired through `make conformance-ext`.
- **Shipped:** OpenAPI/OData/policy-xref binding objects are transcript-auditable and include linked M26 approval, M27 observability, and M28 IAM-bridge evidence checks.
- **Next concrete step:** keep M29 bindings stable while adjacent policy-semantics/profile work advances without overclaiming M25 completion.
