...
def _stable_extension_coverage_failures(entries: list[dict[str, str]]) -> list[str]:
    ...
    suite_payloads = {p: _load_json(p) for p in ext_suites}
    ...
    for entry in entries:
        ...
        suite_hits = [
            p for p, payload in suite_payloads.items()
            if isinstance(payload, dict) and payload.get("extension_id") == ext_id
        ]
        if not suite_hits:
