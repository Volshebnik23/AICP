...
ROADMAP_M30_COMPLETED_RE = re.compile(r"(?m)^###\s*✅\s*M30\b")
...
def main() -> int:
    ...
    m30_completed_in_roadmap = bool(ROADMAP_M30_COMPLETED_RE.search(roadmap))
    if m30_completed_in_roadmap:
        if re.search(r"(?m)^##\s*M30\b", backlog):
            errors.append("AICP_Backlog must not include '## M30' once ROADMAP.md marks M30 completed")
        elif re.search(r"(?m)^##\s+.*\bM30\b", backlog):
            errors.append("AICP_Backlog must not reintroduce M30 as a remaining milestone once completed in ROADMAP.md")
