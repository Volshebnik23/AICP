...
6. Prefer the smallest shipped profile that satisfies your need before adding optional governance overlays.
...
- **Experimental Tier-0 security variant:** select `AICP-AUTHENTICATED-BASE@0.1` when every
...
| Bilateral exchange requiring authenticated senders | `AICP-AUTHENTICATED-BASE@0.1` (experimental) | Core + authenticated-message security suite | CAPNEG, identity/trust/witness profiles as separately needed | Supplied Ed25519 verification material; key discovery/custody remains external |
...
- **Cryptographically sender-authenticated gateway:** use experimental
