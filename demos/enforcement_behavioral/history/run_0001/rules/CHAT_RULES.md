...
- Inconclusive branch: `INCONCLUSIVE` blocks delivery and emits `ALERT(code=POLICY_INCONCLUSIVE, severity=WARNING, recommended_actions=[ESCALATE, ACK_REQUIRED])`.
- Resume behind-head branch: `RESUME_RESPONSE(status=NEEDS_RESYNC)` emits `ALERT(code=RESYNC_REQUIRED, severity=WARNING, recommended_actions=[RETRY, REMEDIATE])`.
