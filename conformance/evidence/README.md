...
current TCK:     AICP-EVIDENCE-TCK-1.11.0
```
...
`AICP-EVIDENCE-TCK-1.11.0`. Frozen 1.2.0 and 1.3.0 releases are historical and
...
scenarios respectively and all required-suite transcripts exactly once. M67 expands the
consumer corpus to 30 mediated, 20 resumable, and 32 delegated cases (82 executions in
total), derived from the exact required suites. Producer counts remain 10, 9, and 13.
...
1.11. A full run launches the same implementation build in both roles and repeats both
...
by the later poll-2. Release records 1.0.0 through 1.10.0 are byte-frozen. Release-specific registry
snapshots make `tck_release.registry_digest` immutable: 1.1.0 remains strong-eligible for
its exact projection reports, while 1.4.0, 1.8.0, and 1.10.0 remain strong-eligible for
their exact reports.
...
- Report 2.0 remains valid for exact frozen projection-v1 releases. Report 2.1 adds discriminated
  ...
  historical/ineligible due to the corrected semantic-authority defect. TCK 1.10 restored
  the canonical normative authority and is frozen historical/eligible. New TCK 1.11
  executions retain report 2.2, trace v4, and six targets while expanding the security and
  adversarial consumer corpus. The frozen historical projection report at
