...
def _run_binding_suite(suite: dict[str, Any], schema: dict[str, Any] | None) -> dict[str, Any]:
    case_validator = Draft202012Validator(schema) if (schema is not None and Draft202012Validator is not None) else None
    core_schema = load_json(ROOT / "schemas/core/aicp-core-message.schema.json")
    core_validator = Draft202012Validator(core_schema) if Draft202012Validator is not None else None
