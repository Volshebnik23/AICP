...
def _run_binding_suite(suite: dict[str, Any], schema: dict[str, Any] | None) -> dict[str, Any]:
    ...
    for rel_case in suite.get("cases", []):
        ...
        if "TB-CP-ORDERING-01" in enabled_checks:
            cp = case_obj.get("channel_properties")
            if isinstance(cp, dict) and cp.get("CP-ORDERING-0.1") == "ordered" and len(extracted_messages) >= 2:
                for idx in range(1, len(extracted_messages)):
                    prev = extracted_messages[idx - 1]
                    cur = extracted_messages[idx]
                    prev_hash = prev.get("message_hash")
                    cur_prev = cur.get("prev_msg_hash")
                    if not isinstance(prev_hash, str) or not prev_hash:
                        add_failure(failures, "TB-CP-ORDERING-01", "ordered mode requires prior embedded message_hash", rel_case, None)
                        continue
                    if not isinstance(cur_prev, str) or not cur_prev:
                        add_failure(failures, "TB-CP-ORDERING-01", "ordered mode requires prev_msg_hash on subsequent embedded messages", rel_case, None)
                        continue
                    if cur_prev != prev_hash:
                        add_failure(
                            failures,
                            "TB-CP-ORDERING-01",
                            f"ordered embedded chain mismatch at index {idx}: prev_msg_hash={cur_prev} expected={prev_hash}",
                            rel_case,
                            None,
                        )
