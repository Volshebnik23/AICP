...
def _run_binding_suite(suite: dict[str, Any], schema: dict[str, Any] | None) -> dict[str, Any]:
    ...
    case_meta: list[tuple[str, str, dict[str, Any]]] = []
        ...
        if "TB-HTTP-RATELIMIT-01" in enabled_checks and case_obj.get("operation") == "overload":
            ...
            if isinstance(response, dict) and response.get("status") == 429:
                ...
                if "Retry-After" not in headers:
                    add_failure(failures, "TB-HTTP-RATELIMIT-01", "overload 429 requires Retry-After header", rel_case, None)
                if not any(h in headers for h in ["RateLimit-Limit", "RateLimit-Remaining", "RateLimit-Reset"]):
                    add_failure(failures, "TB-HTTP-RATELIMIT-01", "overload 429 requires at least one RateLimit-* header", rel_case, None)

        if "TB-HTTP-AUTH-01" in enabled_checks:
            auth = case_obj.get("auth")
            if isinstance(auth, dict):
                token = auth.get("token")
                headers = ((case_obj.get("http_request") or {}).get("headers")) if isinstance(case_obj.get("http_request"), dict) else None
                authorization = headers.get("Authorization") if isinstance(headers, dict) else None
                expected = f"Bearer {token}" if isinstance(token, str) else None
                if not isinstance(expected, str) or authorization != expected:
                    add_failure(failures, "TB-HTTP-AUTH-01", "auth-present case must include Authorization: Bearer <token>", rel_case, None)

        if "TB-SSE-RECONNECT-01" in enabled_checks:
            reconnect_of = case_obj.get("reconnect_of")
            if isinstance(reconnect_of, str):
                base_case = seen_cases_by_id.get(reconnect_of)
                if base_case is None:
                    add_failure(failures, "TB-SSE-RECONNECT-01", f"reconnect_of references unknown or later case_id '{reconnect_of}'", rel_case, None)
                elif base_case.get("operation") != "ssePullMessages":
                    add_failure(failures, "TB-SSE-RECONNECT-01", "reconnect_of must reference ssePullMessages case", rel_case, None)
                else:
                    events = base_case.get("sse_events_out") if isinstance(base_case.get("sse_events_out"), list) else []
                    messages_events = [e for e in events if isinstance(e, dict) and e.get("event") == "messages"]
                    if not messages_events:
                        add_failure(failures, "TB-SSE-RECONNECT-01", "referenced SSE case must include at least one messages event", rel_case, None)
                    else:
                        last_msg_event = messages_events[-1]
                        data = last_msg_event.get("data") if isinstance(last_msg_event.get("data"), dict) else {}
                        expected_cursor = last_msg_event.get("id") if isinstance(last_msg_event.get("id"), str) and last_msg_event.get("id") else data.get("cursor_after_last")
                        request = case_obj.get("http_request") if isinstance(case_obj.get("http_request"), dict) else {}
                        ...
                        if not isinstance(expected_cursor, str) or not expected_cursor:
                            add_failure(failures, "TB-SSE-RECONNECT-01", "referenced case missing reconnect cursor evidence", rel_case, None)
                        else:
                            if headers.get("Last-Event-ID") != expected_cursor:
                                add_failure(failures, "TB-SSE-RECONNECT-01", "Last-Event-ID must equal referenced stream cursor", rel_case, None)
                            if query.get("after") != expected_cursor:
                                add_failure(failures, "TB-SSE-RECONNECT-01", "query.after must equal referenced stream cursor", rel_case, None)

        case_id = case_obj.get("case_id")
        if isinstance(case_id, str) and case_id:
            seen_cases_by_id[case_id] = case_obj
            case_meta.append((case_id, rel_case, case_obj))
    ...
    if "TB-HTTP-SESSION-01" in enabled_checks:
        ...
        for case_id, rel_case, case_obj in case_meta:
            if case_obj.get("operation") == "createSession":
                body = ((case_obj.get("http_response") or {}).get("body")) if isinstance(case_obj.get("http_response"), dict) else None
                if isinstance(body, dict) and isinstance(body.get("session_id"), str):
                    created_session_id = body.get("session_id")
                elif isinstance(case_obj.get("session_id"), str):
        ...
        if created_session_id:
            ...
            for case_id, rel_case, case_obj in case_meta:
                if case_obj.get("operation") == "createSession":
                    continue
                expected = created_session_id
                req = case_obj.get("http_request") if isinstance(case_obj.get("http_request"), dict) else {}
                path = req.get("path") if isinstance(req.get("path"), str) else ""
                body = req.get("body") if isinstance(req.get("body"), dict) else {}
                top_sid = case_obj.get("session_id") if isinstance(case_obj.get("session_id"), str) else None
                body_sid = body.get("session_id") if isinstance(body.get("session_id"), str) else None
                path_sid = None
                marker = '/sessions/'
                if marker in path:
                    tail = path.split(marker,1)[1]
                    path_sid = tail.split('/',1)[0] if tail else None
                for sid in [top_sid, body_sid, path_sid]:
                    if sid is not None and sid != expected:
                        mismatches.append(f"{case_id} ({rel_case}) -> {sid}")
                        break
            if mismatches:
                add_failure(failures, "TB-HTTP-SESSION-01", f"session_id mismatch against createSession '{created_session_id}': {mismatches}", "conformance/bindings/TB_HTTP_WS_0.1.json", None)
