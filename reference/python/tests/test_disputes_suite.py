...
SUITE_PATH = ROOT / "conformance/extensions/DS_DISPUTES_0.1.json"
...
def test_disputes_suite_passes() -> None:
    report = ROOT / "conformance/report_ext_disputes_test.json"
    result = _run_suite(SUITE_PATH, report)
    assert result.returncode == 0, result.stdout + result.stderr
    payload = json.loads(report.read_text(encoding="utf-8"))
    assert payload["passed"] is True
    assert "AICP-EXT-DISPUTES-0.1" in payload.get("compatibility_marks", [])
