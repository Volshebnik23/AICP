...
def test_interop_matrix_tool_with_submission(tmp_path: Path) -> None:
    ...
    assert "impl-one" in md
    assert any(i.get("implementation", {}).get("implementation_id") == "impl-one" for i in data["implementations"])
    impl = next(i for i in data["implementations"] if i.get("implementation", {}).get("implementation_id") == "impl-one")
    assert "AICP-Profile-BASE-0.1" in impl["compatibility_marks"]
