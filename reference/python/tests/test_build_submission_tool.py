...
from interop_submission_validation import INTEGRITY_FILENAME, load_schema_and_registry, validate_common_rules, validate_integrity_manifest, validate_schema
...
def test_build_submission_tool_single_impl_package_with_integrity(tmp_path: Path) -> None:
    ...
    integrity = json.loads((package_dir / INTEGRITY_FILENAME).read_text(encoding="utf-8"))
    ...
    assert [entry["path"] for entry in integrity["tracked_files"]] == [
        "submission.json",
        "reports/report_profile_base.json",
        "reports/report_core.json",
    ]
    assert _validate_package(package_dir) == []
...
def test_build_submission_tool_pairwise_package_validates_with_integrity(tmp_path: Path) -> None:
    report_one = _write_json(
...
def test_integrity_validation_detects_tampering(tmp_path: Path) -> None:
    report = _write_json(
        tmp_path / "inputs" / "report_profile_base.json",
        {"profile_id": "AICP-BASE", "passed": True},
    )
    out_root = tmp_path / "submissions"
    result = _run_tool(
        "--out-root",
        ...
        "PF_AICP_BASE_0.1",
        "--disclosure",
        "Fictional local package; not a market-facing claim.",
        "--with-integrity",
    )
    assert result.returncode == 0, result.stderr

    package_dir = out_root / "fictional-drift"
    (package_dir / "reports" / "report_profile_base.json").write_text('{"profile_id":"AICP-BASE","passed":false}', encoding="utf-8")

    status, errors = validate_integrity_manifest(package_dir)
