...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--with-integrity",
        action="store_true",
        help="Write bundle-integrity.json for files copied into the generated package",
    )
    parser.add_argument(
        "--validate",
        action="store_true",
        help="Validate the generated package with the existing submission schema/common-rule/integrity checks",
    )
...
def _build_manifest(args: argparse.Namespace, report_refs: list[str], evidence_types: list[str], generated_at: str) -> dict[str, Any]:
...
def _write_package(package_dir: Path, report_paths: list[Path], manifest: dict[str, Any]) -> list[str]:
    ...
    return ["submission.json", *[f"reports/{report_path.name}" for report_path in report_paths]]


def _write_integrity_manifest(package_dir: Path, *, submission_id: str, tracked_paths: list[str], generated_at: str) -> None:
    ...
    (package_dir / INTEGRITY_FILENAME).write_text(json.dumps(integrity, indent=2) + "\n", encoding="utf-8")
...
def _validate_package(package_dir: Path) -> list[str]:
    ...
    integrity_status, integrity_errors = validate_integrity_manifest(package_dir)
...
def main() -> int:
    ...
    manifest = _build_manifest(args, report_refs, evidence_types, generated_at)

    package_dir.mkdir(parents=True, exist_ok=False)
    try:
        tracked_paths = _write_package(package_dir, report_paths, manifest)
    ...
    if args.with_integrity:
        print(f"[OK] wrote integrity manifest -> {package_dir / INTEGRITY_FILENAME}")
    if args.validate:
