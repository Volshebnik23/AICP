    ...
    try {
      ...
      if (request.operation === "describe") {
        ...
        if (!sessions.has(session)) sessions.set(session, {
          cursor: "c0", visible: processVisibleHashes, proposal: null, acceptance: null, attestation: null, challenge: null,
        });
        ...
        if (phase === "propose") {
          ...
          if (phase === "poll_attestation" && ["stale_final_poll_cursor", "hardcoded_c0_final_poll"].includes(behavior)) args.after_cursor = "c0";
          if (behavior === "unrelated_cursor") args.after_cursor = "c-unrelated";
          if (behavior === "missing_poll_limit") delete args.limit;
          if (behavior === "wrong_poll_limit") args.limit = 2;
          if (behavior === "wrong_poll_session") args.session_id = `${session}-wrong`;
          expectedType = {
