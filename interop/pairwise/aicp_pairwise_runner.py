#!/usr/bin/env python3
"""Run two fresh bidirectional MCP exchanges between two named clean-room peers."""
    ...
    for direction in run["directions"]:
        ...
        for item in messages:
            ...
            if message["message_type"] == "CONTRACT_PROPOSE":
                shape = {"contract_roles": payload["contract"].get("roles"), "has_contract_hash": isinstance(payload.get("contract_hash"), str)}
...
def load_release() -> tuple[dict[str, Any], str]:
    ...
    if not isinstance(release, dict):
        raise ValueError(f"{RELEASE_ID} is not registered")
    snapshot_path = HERE / "release_registry_snapshots" / f"{RELEASE_ID}.json"
    snapshot = json.loads(snapshot_path.read_text(encoding="utf-8"))
    frozen = next((item for item in snapshot.get("releases", []) if item.get("release_id") == RELEASE_ID), None)
    if frozen != release:
        raise ValueError(f"{RELEASE_ID} current record differs from its immutable snapshot")
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        "report_format_version": "1.0", "report_type": "aicp.pairwise_joint_execution", "pairwise_tck_release": tck,
