#!/usr/bin/env python3
"""Independently evaluate Pairwise TCK 1.1 joint reports."""
...
ROOT = HERE.parents[1]
for import_path in (HERE,):
    if str(import_path) not in sys.path:
        sys.path.insert(0, str(import_path))

from pairwise_semantic_normalizer_v1_1 import semantic_digest  # noqa: E402
...
def schema_errors(report: dict[str, Any]) -> list[dict[str, str]]:
    ...
    schema = json.loads((HERE / "pairwise_joint_report_v1_1.schema.json").read_text(encoding="utf-8"))
...
def load_release_errors(report: dict[str, Any]) -> tuple[dict[str, Any] | None, list[dict[str, str]]]:
    ...
    for field in ("report_schema", "evaluator", "normalizer", "target_registry", "scenario_catalog"):
        ...
        if not path.is_file() or artifact.get("content_digest") != sha256_file(path):
            errors.append(error("PAIRWISE_TCK_ARTIFACT_DRIFT", f"{field} no longer matches the immutable release"))
        schema_path_ref = artifact.get("schema_path")
        if isinstance(schema_path_ref, str):
            schema_path = ROOT / schema_path_ref
            if not schema_path.is_file() or artifact.get("schema_digest") != sha256_file(schema_path):
                errors.append(error("PAIRWISE_TCK_ARTIFACT_DRIFT", f"{field} schema no longer matches the immutable release"))
    if release.get("registry_schema_digest") != sha256_file(HERE / "tck_releases_v2.schema.json"):
        errors.append(error("PAIRWISE_TCK_ARTIFACT_DRIFT", "Pairwise registry v2 schema drifted"))
    for bundle_field in ("runner_bundle", "evaluator_bundle"):
        ...
        if not bundle_path.is_file() or bundle_ref.get("digest") != sha256_file(bundle_path):
            errors.append(error("PAIRWISE_TCK_ARTIFACT_DRIFT", f"{bundle_field} manifest drifted"))
            continue
        bundle = json.loads(bundle_path.read_text(encoding="utf-8"))
        for entry in bundle.get("entries", []):
            path = ROOT / str(entry.get("path", ""))
            if not path.is_file() or entry.get("digest") != sha256_file(path):
                errors.append(error("PAIRWISE_TCK_ARTIFACT_DRIFT", f"{bundle_field} closure drifted: {entry.get('path')}"))
    ...
    for participant in participants if isinstance(participants, list) else []:
        ...
        for report_kind in ("profile_report", "binding_report"):
            ...
            if not isinstance(subject, dict) or any(subject.get(key) != value for key, value in expected_identity.items()):
                ...
                continue
            kind = "profile" if report_kind == "profile_report" else "binding"
            for detail in evaluate_side_report(side_report, kind=kind, identity=expected_identity):
                code = "PAIRWISE_PROFILE_REPORT_INELIGIBLE" if kind == "profile" else "PAIRWISE_BINDING_REPORT_INELIGIBLE"
                errors.append(error(code, f"side {side}: {detail}"))
    return errors
    ...
    *,
    ...
    if not isinstance(payload, dict):
        ...
        if (
            ...
            or contract.get("contract_id") != direction.get("contract_id")
            or contract.get("roles") != ["initiator", "responder"]
            or contract.get("goal") != run.get("challenge")
            or payload.get("contract_hash") != frozen_hash("contract", contract).get("object_hash")
        ):
        ...
        expected_result = frozen_hash("result", {"peer_hash": expected_previous}).get("object_hash")
    ...
    exact_control_bindings = {
        ...
        "side": sender,
        "session_id": direction.get("session_id"),
        "contract_id": direction.get("contract_id"),
        "message_id": message.get("message_id"),
        "timestamp": message.get("timestamp"),
    }
    if any(control_input.get(field) != value for field, value in exact_control_bindings.items()):
        errors.append(error("PAIRWISE_CONTROL_RUN_BINDING_INVALID", prefix))
    ...
    for run in runs:
        ...
        for direction in directions:
            ...
            for sequence, item in enumerate(messages, start=1):
                ...
                errors.extend(validate_exchange(item, sequence=sequence, direction=direction, participants=participants, previous=previous, run=run, seen_rpc_ids=seen_rpc_ids))
                previous = item.get("message")
            core_errors = validate_core_transcript(
                [item.get("message", {}) for item in messages if isinstance(item, dict)]
            )
            errors.extend(
                error("PAIRWISE_CORE_TRANSCRIPT_INVALID", f"{direction.get('direction')}: {item.get('test_id')}: {item.get('message')}")
