#!/usr/bin/env python3
"""Run Pairwise TCK 1.3 with raw-role, global-causality, and cursor evidence."""
...
def verify_runner_bundle(*, bundle_path: Path | None = None, seed: Path | None = None) -> None:
    path = Path(bundle_path or HERE / "pairwise_runner_bundle_v1_3.json")
    bundle = json.loads(path.read_text(encoding="utf-8"))
    entries = bundle.get("entries")
    if bundle.get("release_id") != RELEASE_ID or not isinstance(entries, list):
        raise ProcessBoundaryError("Pairwise 1.3 runner bundle is malformed")
    ...
    expected = {
        ...
        for item in entries
        if isinstance(item, dict) and item.get("role") == "in_process_runner_import_closure"
    }
    if actual != expected:
        raise ProcessBoundaryError("runtime Pairwise 1.3 runner import closure differs from its frozen bundle")
...
def relay_phase(
    *,
    ...
    global_sequences["event"] += 1
    client_events.append(
        {
            "sequence": len(client_events) + 1,
            "global_event_sequence": global_sequences["event"],
    ...
    global_sequences["exchange"] += 1
    exchanges.append(
        {
            "sequence": exchange_sequence,
            "global_exchange_sequence": global_sequences["exchange"],
            "phase": phase,
            "operation": operation,
...
def load_release() -> tuple[dict[str, Any], str]:
    ...
    bundle_ref = release.get("side_authority_bundle")
    if not isinstance(bundle_ref, dict) or not isinstance(bundle_ref.get("path"), str):
        raise ValueError("Pairwise 1.3 side-authority bundle reference is missing")
    bundle_path = (ROOT / bundle_ref["path"]).resolve()
    if (
        not bundle_path.is_relative_to(ROOT.resolve())
        or not bundle_path.is_file()
        or sha256_file(bundle_path) != bundle_ref.get("digest")
    ):
        raise ValueError("Pairwise 1.3 side-authority bundle manifest drift")
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for run_number in (1, 2):
        ...
        global_sequences = {"event": 0, "exchange": 0}
        resources: list[JsonLineProcess] = []
        with tempfile.TemporaryDirectory(prefix="aicp-pairwise-v1-3-") as temporary:
